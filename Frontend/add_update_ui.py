...
def add_update():
    ...
    get_response = requests.get(f"{API_URL}/expenses/{selected_date}")
    if get_response.status_code == 200:
        existing_expenses = get_response.json()
    else:
        ...
        for i in range(5):
            ...
            with col2:
                category_input = st.selectbox(label="Category", options=categories,
                                              index=categories.index(category.title()), key=f"categories_{i}",
                                              label_visibility="collapsed")
            with col3:
